...
module.exports = function SPCore(mod) {
	...
	function startSkill(type, version, code, data) {
		...
		// Early return for conditions that do not need delay
		...
		if (delayNext && Date.now() <= stageEndTime) {
			...
			if (delay) {
				// Use a callback function to avoid duplicating logic
				const delayedSkillCallback = () => {
					if (handleStartSkill(type, event, info, data) !== false) toServerLocked(data);
				};

				delayedSkillTimeout = mod.setTimeout(delayedSkillCallback, delay);

				// Early return to avoid unnecessary execution
				return false;
			}
		}

		// Directly invoke handleStartSkill without an unnecessary variable
		return handleStartSkill(type, event, info, data);
	}
	// End of updated code
	...
	function handleStartSkill(type, event, info, data) {
		...
		const currentSkillCategory = skillsClient[player.templateId]?.[skill.id]?.category
				...
				for (let effect of abnormal.effects) {
					switch (effect.type) {
						...
						case 317:
							...
							break;
						case 209:
							enabledCategories.add(...abnormal.bySkillCategory);
							break;
						case 210:
							disabledCategories.add(...abnormal.bySkillCategory);
							break;
						case 235:
							if (hasOverlapCategory(abnormal.bySkillCategory)) {
								skillSpeed *= effect.value;
							}
							break;
						case 236:
							if (hasOverlapCategory(abnormal.bySkillCategory)) {
						...
						case 239:
							if (hasOverlapCategory(abnormal.bySkillCategory)) {
								switch (effect.method) {
									case 1:
										staminaA = effect.value;
										break;
									case 2:
										staminaB += effect.value;
										break;
									case 3:
										staminaC += effect.value - 1;
										break;
									case 4:
										staminaD *= effect.value;
										break;
								}
							}
							break;
			...
			// Modified hasOverlapCategory function for efficiency
		...
		function runPassive(passive) {
			...
			switch (passive.type) {
				case 77:
					effectScale += Math.max(0, passive.method === 3 ? passive.value - 1 : 0);
					break;
				case 82:
					staminaA = passive.method === 1 ? passive.value : staminaA;
					staminaB += passive.method === 2 ? passive.value : 0;
					staminaC += passive.method === 3 ? passive.value - 1 : 0;
					break;
				case 218:
					skillSpeedMult += passive.value - 1;
					break;
				case 220:
					chargeSpeed += passive.value - 1;
					break;
			}
		}
